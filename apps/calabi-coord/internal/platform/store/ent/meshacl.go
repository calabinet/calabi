...
// MeshACL is the model entity for the MeshACL schema.
type MeshACL struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// owning meshnet == org id; one ACL doc per meshnet
	MeshnetID int64 `json:"meshnet_id,omitempty"`
	// the ACL document as JSON (core.ACLPolicy)
...
func (*MeshACL) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case meshacl.FieldID, meshacl.FieldMeshnetID:
...
// MeshACLs is a parsable slice of MeshACL.
type MeshACLs []*MeshACL
