...
- `calabi-coord` 是组网的协调器：谁在网里、每台设备分到什么地址、谁可以访问谁。

两种模式都不需要在你的机器上开任何入站端口：连接都是客户端拨出去的。

### 这个仓库里有什么

数据面：`calabi` 客户端、`calabi-edge` 数据节点、`calabi-coord` 组网协调器。
...
- **中继是你自己的**——`calabi-edge` 配 `role: relay` 就是中继。它按设备公钥
...
#    不设 CALABI_COORD_DB_DSN 的话设备注册表在内存里：协调器一重启，
#    每台设备都会重新入网并换一个 100.64.x.x 地址。
...
然后在设备之间 `ping 100.64.0.x` —— 任何一端都不用做端口映射。

> **设备到协调器这段的 TLS。** 客户端默认用 TLS 连协调器，并用编译进二进制的 CA
> 校验它。自己部署时有两条路：给 `calabi-coord` 签一张你自己 CA 的证书，设备侧用
