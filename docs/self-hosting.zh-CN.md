...
没有任何遥测、没有任何统计代码，整个客户端只有**一个**硬编码的非本地地址：
`download.calabi.net` 上那份带签名的更新清单。只有一条路径会碰到它——装成**全机服务**
（`daemon install --system`）的客户端，每 6 小时查一次——而 standalone 的守护进程在到
那段代码之前就被分流走了。如果你希望它从构造上就不存在、而不是靠分流，安装时把
