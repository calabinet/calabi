...
**控制面**（账号、组织、计费、托管的全球边缘节点车队）是另一个闭源的托管产品。
...
托管版 Calabi 在这之上还加了什么——账号、托管的全球边缘车队、计费——列在下面的
*自建拿不到什么*。
...
> 这些以前是平台专属的，现在不是了：只有一份二进制，自建的边缘节点执行的策略集
> 和托管的完全一样。托管产品里同样的功能是按**套餐**门控的，而那道门在控制面
> （tunnel-svc）里，不在这份代码里——所以取消 edition 切分并没有把付费功能
> 白送给托管用户。
