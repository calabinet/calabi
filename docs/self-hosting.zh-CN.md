...
这三个程序就是数据面本身——不回连、不需要账号，完全跑在你自己的机器上。
（`apps/client-android` 里的 Android App 是 calabi.net 的客户端，目前还不能加入自建的协调器。）
...
**客户端会往外打什么。** standalone 模式下它只拨你的边缘节点，别的都不拨。这棵树里
没有任何统计代码，整个客户端只有**一个**硬编码的非本地地址：
`download.calabi.net` 上那份带签名的更新清单。只有一条路径会碰到它——平台版守护进程，
每 6 小时查一次——而 standalone 的守护进程在到那段代码之前就被分流走了。平台版守护进程
还会把自己的自更新结果（从哪个版本到哪个版本、失败在哪一步、报错内容）报给它登录的
控制面，走的是注册设备的同一条连接；standalone 守护进程没有更新代理，也就没有可报的。如果你希望它从构造上就不存在、而不是靠分流，安装时把
`CALABI_UPDATE_MANIFEST=` 设成空字符串，或者指向你自己的镜像。

> **模式必须写在守护进程读得到的地方。** `calabi mode standalone` 存进的是**你自己**的
> 配置目录，而装好的服务读的是它自己的（二进制旁边，`--system` 则是全机那个）。用
> `--config` 装本地 supervisor（见下文）就不用管这件事——那个服务无论如何都跑本地守护
> 进程。而 `calabi daemon install` **不带** `--config` 时，在 standalone 模式下会直接
> 拒绝，而不是给你装一个起来就去连控制面的服务。
...
只要本地守护进程在跑，浏览器打开 **http://127.0.0.1:7400** 就是一个面板：

- 隧道列表 + 实时流量计数；
- **请求检查器**（按连接的日志 + HTTP 请求/响应抓取）；
- 守护进程日志；
- 以及**新建 / 删除隧道、实时改每条隧道的安全策略**，改动写回你的 `tunnels.yaml`。

一次性的 `calabi http 8080` 也会占同一个端口（被占就顺延），但它发的是一个
**纯状态页**：隧道、公网地址、字节计数，就这些。上面那个面板是守护进程的——
检查器、日志查看、增删改都跑在守护进程的本地 API 上，而一次性命令不会启动它。
